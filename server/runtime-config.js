...
function isAllowedWebSocketOrigin(request, allowedOrigins = new Set(), trustProxy = false) {
  ...
  try { return new URL(origin).host.toLowerCase() === forwardedHost(request, trustProxy).toLowerCase(); }
...
module.exports = { booleanEnv, clientAddress, commaSeparatedSet, forwardedHost, integerEnv, isAllowedWebSocketOrigin, runtimeConfig, securityHeaders };
