...
const CLIENT_MESSAGE_TYPES = Object.freeze([
  ...
  'chat', 'clear_ai', 'create_room', 'edit_preview_port', 'edit_preview_tile', 'fill_ai',
...
const LOBBY_RECOVERY_MESSAGE_MAX_BYTES = 4 * 1024 * 1024;
...
function parseClientMessage(data) {
  ...
  const byteLimit = value.type === 'texture_pack_publish'
    ? TEXTURE_PACK_MESSAGE_MAX_BYTES
    : value.type === 'recover_lobby'
      ? LOBBY_RECOVERY_MESSAGE_MAX_BYTES
      : REGULAR_MESSAGE_MAX_BYTES;
  if (byteLength > byteLimit) return { ok: false, error: 'Message is too large.' };
  return { ok: true, value };
}

module.exports = { CLIENT_MESSAGE_TYPES, LOBBY_RECOVERY_MESSAGE_MAX_BYTES, REGULAR_MESSAGE_MAX_BYTES, TEXTURE_PACK_MESSAGE_MAX_BYTES, parseClientMessage };
